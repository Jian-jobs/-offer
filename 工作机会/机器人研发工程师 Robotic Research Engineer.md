...
分为四部分：
第一部分（总共10分钟）：
10道题——10分：文章大意题。读一段长句子，选择大概意思，每道题75秒——60秒；
第二部分（总共10分钟）：
10道题——10分：图标计算题。会让你计算一些增长率了之类的，或者根据图表从选项中选择正确的；75秒——60秒；计算来不及细算，算个大概，选择选项。
第三部分（总共10分钟）：
10道题——10分：找规律题。根据给出的图形，推选下一个图形。75秒——60秒；
第四部分（没有时间限制）：
98道题：素质性格题。没有时间限制，纯选择符合自己性格的选择。
